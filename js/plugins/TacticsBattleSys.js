...
'use strict';
  ...
  Game_Action.prototype.makeDamageValue = function(target, evaluate, critical) {
    ...
    for(var id = 1; id < $dataStates.length; id++){
      if (target.isStateAffected(id)) {
        ...
        if(stoneskin){
          value = Math.max(value - target.hp * parseInt(stoneskin) / 100, 0);
        }
      }
    }
  ...
  Game_BattlerBase.prototype.meetsSkillConditions = function(skill) {
    if($gameMap.isReservationAction()){ //予約アクションではMPTPは消費しないので条件を変える
  ...
  Game_Map.prototype.isBattleActivate = function() {
  ...
  // SRPG機能を有効にします
  Game_Map.prototype.battleActivate = function() {
 ...
 // SRPG機能を無効にします
  Game_Map.prototype.battleDeactivate = function() {
    this.initColorArea();
  ...
  // SRPGユニットを返す
  Game_Map.prototype.units = function(alive) {
  ...
  */
  ...
  Game_Map.prototype.isAllyTurn = function() {
    return this._isAllyTurn;
  };
  // 敵ターンかどうかを返す(行動順が回ってきた際にtrueに処理する)
  Game_Map.prototype.isEnemyTurn = function() {
    return this._isEnemyTurn;
  };
  //ターン開始前の処理
  Game_Map.prototype.startTurn = function(turnUnit) {
    //マップ上にいるユニットのすり抜け設定
    for(var i = 0; i < this.unitList().length; i++){
  ...
  // ターンを終了する
  Game_Map.prototype.endTurn = function() {
    this._isAllyTurn = false;
    this._isEnemyTurn = false;
  ...
  //行動順調整用スクリプト
  Game_Map.prototype.setWtTurnList = function() {
  ...
  //ターンリストを行動順にソート
  Game_Map.prototype.wtTurnListSort = function() {
    //WTソート
  ...
  */
  ...
  Game_Map.prototype.addReservationActionList = function(attacker, skill, target, type) {
    var reservationAction = [attacker, skill, target, type];
    this._reservationActionList.push(reservationAction);
  }
  
  // 予約されたアタッカーユニットの読み込み
  Game_Map.prototype.loadReservationAttackUnit = function() {
    var reservationAction = this._reservationActionList[0];
    return reservationAction[0];
  }
  // 予約された使用スキルの読み込み
  Game_Map.prototype.loadReservationUseSkill = function() {
    var reservationAction = this._reservationActionList[0];
    return reservationAction[1];
  }
  // 予約された対象者の読み込み
  Game_Map.prototype.loadReservationTargetUnit = function() {
    var reservationAction = this._reservationActionList[0];
    return reservationAction[2];
  }
  // 予約されたタイプの読み込み
  Game_Map.prototype.loadReservationUseSkillType = function() {
    var reservationAction = this._reservationActionList[0];
    return reservationAction[3];
  }
  // 予約されたアクションの削除
  Game_Map.prototype.removeReservationActionList = function() {
  ...
  // 予約されたアクションタイプが正しいか
  Game_Map.prototype.isReservationActionType = function(type) {
  ...
  // 予約されたアクションターンを開始
  Game_Map.prototype.startReservationActionTurn = function() {
    $gameTemp._isReservationActionTurn = true;;
  }
  // 予約されたアクションターンを終了
  Game_Map.prototype.endReservationActionTurn = function() {
    $gameTemp._isReservationActionTurn = false;;
  }
  // 予約されたアクションのターンか
  Game_Map.prototype.isReservationActionTurn = function() {
    return $gameTemp._isReservationActionTurn;
  }
  
  // 予約されたアクション数の確認(デバッグ用)
  Game_Map.prototype.isReservationActionCount = function() {
  ...
  */
  // マップ上にいるステート
  Game_Map.prototype.stateAction = function() {
    //マップ上にいるユニットのステートをチェックする
    for(var i = 0; i < this.unitList().length; i++){
      var target = this.unitList()[i];
      ...
      if (this.isActor().isStateAffected(id)) {
        if($dataStates[id].meta.activate){
          if($dataStates[id].meta.activate == "trap" && $dataStates[id].meta.trapChase) {
            ...
            //マップ上にいるユニットのステートをチェックする
            for(var j = 0; j < $gameMap.unitList().length; j++){
              var trapGrantorUnit = $gameMap.unitList()[j];
              var trapGrantorActor = trapGrantorUnit.isActor();
      ...
      if (actor.isStateAffected(id)) {
        ...
        if($dataStates[id].meta.activate){
          if($dataStates[id].meta.activate == "trap") {
            ...
            if(trapGrantor){
              if(trapGrantor == "self"){ //トラップを付与されたキャラのステータス依存のトラップ(味方全体に効果が及ぶようにするとエラーが起こる)
                $gameMap.addReservationActionList(this,$dataSkills[parseInt($dataStates[id].meta.skill)],this,$dataStates[id].meta.activate);
              }else{
                //マップ上にいるユニットのステートをチェックする
                for(var j = 0; j < $gameMap.unitList().length; j++){
                  var trapGrantorUnit = $gameMap.unitList()[j];
  ...
  Game_Event.prototype.canPass = function(x, y, d) {
    _Game_CharacterBase_canPass.call(this);
  ...
  */
  ...
  Game_Player.prototype.hasStepAnime = function() {
    return _Game_Player_hasStepAnime.call(this) || $gameMap.isBattleActivate();
  };

  var _Game_Player_refresh = Game_Player.prototype.refresh;
  Game_Player.prototype.refresh = function() {
    _Game_Player_refresh.call(this);
    if ($gameMap.isBattleActivate() || $gameSwitches.value(3)) {  // SRPG機能が有効なら歩行グラフィックをカーソルにする
  ...
  */
  ...
  Sprite_Character.prototype.update = function() {
    ...
    if(!actor) return;
    if($gameMap._battleActive && !actor.isDead()){
      this.updateDamagePopup();
...
//-----------------------------------------------------------------------------
  ...
  */
  ...
  Scene_Map.prototype.updateMain = function() {
    ...
    if ($gameMap.isUnitAnimationPlaying() || !$gameMap.isBattleActivate() || $gameMap.isEventRunning()) return; //戦闘中以外、イベント実行中は処理をしない
    // 3択
    //ゲームオーバー判定
    ...
    //予約ターン
    if ($gameMap.isReservationAction() && !$gameMap.isAllyTurn() && !$gameMap.isEnemyTurn()) {
  ...
  */
  ...
  Scene_Map.prototype.updateAllyTurn = function() {
    ...
    case 0: //カメラ移動完了後コマンド表示
      ...
      if($gamePlayer.isStopping () && $gameTemp._cameraWait){
        ...
        $gameMap._phaseState = 1;
      }
      break;
    case 1: //ターン開始前処理(アニメーションを取り扱う)
      //ターン開始前の処理
      $gameMap.startTurn(turnUnit);
      turnUnit.beforeTurnStart();
      
      //以下はアニメーション終了後の処理
      $gameMap.setMovableArea(turnUnit);
      //元の座標を記憶
      $gameMap._fromX = turnUnit.x;
      $gameMap._fromY = turnUnit.y;
      $gameMap._fromD = turnUnit.direction();
      $gameMap.setInvisibleArea($gameMap.unitList());
      $gameMap.showInvisibleArea(turnUnit);
      $gameMap._phaseState = 2;
      break;
    case 2: //コマンド選択＆入力処理
      if (!turnUnit.isActor().canMove()){
        $gameMap._phaseState = 11; //麻痺とかであれば以降の処理は行わず次のターンへ
        return;
      }
    ...
    case 4: //移動処理(移動完了したらphaseStateを上げる)
      $gameTemp._commandTime = true;
      if(turnUnit.pos($gameMap._toX, $gameMap._toY)){
        $gameMap.showInvisibleArea(turnUnit);
        this.openCommandWindow();
        this.openBattleStatusWindow(turnUnit);
        $gameMap._phaseState = 2;//コマンド選択へ戻る
      }else{
    ...
    case 9: //コマンド実行処理(対象アニメーション)
      ...
      $gameMap._phaseState = 10;//ダメージ表示フェーズへ移行
      break;
  ...
  Scene_Map.prototype.updateEnemyTurn = function() {
    ...
    case 2: //試行錯誤
      ...
      //if (turnUnit.target()){
        //この時点でコマンドもセットする
        $gameMap._phaseState = 3; //状況によっては5に移行
      //}else{
    ...
    case 3: //移動先選択
      ...
      if(!turnUnit.isActor().meetsSkillConditions($dataSkills[2]) || targetPos == null) {
        $gameMap._phaseState = 5; //移動不可なら対象選択へ
        return;
      }
      $gamePlayer.setCameraXy(targetPos.x, targetPos.y);
      $gameMap._toX = targetPos.x;
      $gameMap._toY = targetPos.y;
      $gameMap._phaseState = 4;
      break;
    case 4: //移動処理(移動完了したらphaseStateを上げる)
    ...
    case 6: //範囲確認
      ...
      if($gameTemp._moveTargetPointFlag || $gameMap.isInsideArea(turnUnit.target().x,turnUnit.target().y)){
        ...
        $gameMap._phaseState = 7;//対象アニメーションフェーズへ移行
      }else{
        //待機という扱いになるためMP回復しターン終了
        this.commandWait();
      }
    ...
    case 7: //コマンド実行処理(詠唱アニメーション)
      ...
      this.showActionMotion(turnUnit);
      //this.showActionAnimation(turnUnit);
      $gameMap._phaseState = 9;//対象アニメーションフェーズへ移行
      break;
    case 9: //コマンド実行処理(対象アニメーション)
      //対象に攻撃アニメーション
      //Game_Eventにて「たたかう」だと対象の装備が反映されたアニメーションが表示される
  ...
  Scene_Map.prototype.updateReservationTurn = function() {
    ...
    case 0: //カメラ移動完了後コマンド表示
      ...
      this.openBattleStatusWindow(turnUnit); //自身のステータスを表示
      //ターン開始前処理(リジェネ回復など)
      $gameMap.startReservationActionTurn();
      $gameMap._phaseState = 1;
      break;
    ...
    case 2: //スキル予約
      $gameMap.setInvisibleArea($gameMap.unitList());
      $gameMap.showInvisibleArea(turnUnit);
      turnUnit.setUseSkill($gameMap.loadReservationUseSkill());
      if((!turnUnit.isActor().meetsSkillConditions(turnUnit.useSkill())) || (!turnUnit.isActor().canMove())) {
        //$gameMessage.add("攻撃不可");
        SoundManager.playBuzzer();//ブザー
        $gameMap._phaseState = 11; //スキル使用不可の場合、この後の処理は行わずターン終了する
        return;
      }
      $gameMap._phaseState = 3;
      break;
    case 3: //予約ターンではスルー
      $gameMap._phaseState = 4;
      break;
    case 4: //予約ターンではスルー
      ...
      break;
    case 5: //対象予約
      turnUnit.setTarget($gameMap.loadReservationTargetUnit());
      $gameMap._phaseState = 6;//範囲確認へ移行
      break;
    ...
    case 11: //ターン終了後処理(アニメーションを取り扱う)
      ...
      $gameMap.endReservationActionTurn();
      $gameMap.removeReservationActionList();//ターン終了前にこのバトルの予約を消さないとループするの削除処理を行う
      $gameMap._phaseState = 12;//事後処理
      break;
    case 12: //事後処理
      this.endTurn(); //
  ...
  Scene_Map.prototype.commandMove = function() {
    var turnUnit = $gameMap._turnUnit;
    $gameMap._phaseState = 3;//移動選択
    //コマンドウインドウを閉じる
    this.closeCommandWindow();
    //移動やり直し用
    turnUnit.setPosition($gameMap._fromX, $gameMap._fromY);
    turnUnit.setDirection($gameMap._fromD);
  ...
  Scene_Map.prototype.commandWait = function() {
    ...
    $gameMap._phaseState = 11;//ターン終了後処理
  };
  ...
  Scene_Map.prototype.updateMove = function() {
    var turnUnit = $gameMap._turnUnit;
    var d = turnUnit.findDirectionTo($gameMap._toX, $gameMap._toY);
    ...
    if (TouchInput.isTriggered() || Input.isPressed('ok')){
      ...
      if(turnUnit.useSkill().scope == 1 || turnUnit.useSkill().scope == 2){
        ...
        if(!turnUnit.target() && $gameMap.isInsideArea(x-1,y)){
          turnUnit.setTarget($gameMap.unitEnemyXy(x-1,y));
        }
        if(!turnUnit.target() && $gameMap.isInsideArea(x+1,y)){
          turnUnit.setTarget($gameMap.unitEnemyXy(x+1,y));
        }
        if(!turnUnit.target() && $gameMap.isInsideArea(x,y-1)){
          turnUnit.setTarget($gameMap.unitEnemyXy(x,y-1));
        }
        if(!turnUnit.target() && $gameMap.isInsideArea(x,y+1)){
          turnUnit.setTarget($gameMap.unitEnemyXy(x,y+1));
        }
        ...
        //以下ターゲット選択時、座標のズレバグ対策
        if(!turnUnit.target() && $gameMap.isInsideArea(x-1,y)){
          turnUnit.setTarget($gameMap.unitAllyXy(x-1,y));
        }
        if(!turnUnit.target() && $gameMap.isInsideArea(x+1,y)){
          turnUnit.setTarget($gameMap.unitAllyXy(x+1,y));
        }
  ...
  Scene_Map.prototype.commandYes = function() {
    ...
    //コマンド実行
    $gameMap._phaseState = 7;//詠唱アニメーションフェーズへ移行
  };
  ...
  */
  ...
  Scene_Map.prototype.showActionMotion = function(unit) {
    ...
    //unit.hasStepAnime();
    $gameMap._phaseState = 10;//対象アニメーションフェーズへ移行
  };
  ...
  Scene_Map.prototype.endTurn = function() {
    ...
    $gameMap._phaseState = 0; //カメラ移動完了後コマンド表示
  };
  ...
  Scene_Map.prototype.startBattle = function() {
    $gameMap.battleActivate();
    $gameSwitches.setValue(2, true);
  };
  
  // 戦闘終了後処理(戦闘不能時)
  Scene_Map.prototype.gameOver = function() {
    $gameMap.battleDeactivate(); //フェードアウト後ユニットを全消去し移動可能な状態でバトルモードを解除してからフェードインする流れが良い
    this.endTurn();
    this._phaseState = 0;
